...
import { resolvePreferredIdleHandsTmpDir } from "../infra/tmp-idlehands-dir.js";
...
async function withOutsideHardlinkInIdleHandsTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside idlehands tmp root",
      media: path.join(os.tmpdir(), "outside-idlehands", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked IdleHands tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked IdleHands tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInIdleHandsTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked IdleHands tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInIdleHandsTmp(
