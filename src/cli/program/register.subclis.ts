...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.IDLEHANDS_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.IDLEHANDS_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<IdleHandsConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live IdleHands docs",
    hasSubcommands: false,
    register: async (program) => {
