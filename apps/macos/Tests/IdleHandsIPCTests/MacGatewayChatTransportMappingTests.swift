...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: IdleHandsProtocol.AnyCodable(["ok": IdleHandsProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: IdleHandsProtocol.AnyCodable(["ok": IdleHandsProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = IdleHandsProtocol.AnyCodable([
            "runId": IdleHandsProtocol.AnyCodable("run-1"),
            "sessionKey": IdleHandsProtocol.AnyCodable("main"),
            "state": IdleHandsProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: IdleHandsProtocol.AnyCodable(["a": IdleHandsProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
