...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: IdleHandsProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: IdleHandsProtocol.AnyCodable] = [
            "presence": .init([IdleHandsProtocol.AnyCodable(entry)]),
        ]
        let payload = IdleHandsProtocol.AnyCodable(payloadMap)
