...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["idlehands"]
        let text = "hey idlehands"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey idlehands do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["idlehands"], minPostTriggerGap: 0.3)
