...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.idlehands", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: IdleHandsPeekabooBridgeServices?
    private static var idlehandsSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("IdleHands", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = IdleHandsPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.idlehandsSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.idlehandsSocketPath, privacy: .public)")
    }
...
@MainActor
private final class IdleHandsPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.idlehands.peekaboo")
