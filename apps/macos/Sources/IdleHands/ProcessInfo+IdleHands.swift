...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.idlehands.mac`) even if the shipped
    ...
    {
        if environment["IDLEHANDS_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "idlehands.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "idlehands.nixMode") { return true }
