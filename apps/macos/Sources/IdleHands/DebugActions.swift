...
enum DebugActions {
    private static let verboseDefaultsKey = "idlehands.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "idlehands.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = IdleHandsPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "IdleHands", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = IdleHandsPaths.configURL
        guard
