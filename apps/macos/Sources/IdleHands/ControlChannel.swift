...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: IdleHandsProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.idlehands", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: IdleHandsKit.AnyCodable]()) {
                $0[$1.key] = IdleHandsKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: IdleHandsProtocol.AnyCodable?) -> [String: IdleHandsProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: IdleHandsProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: IdleHandsKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: IdleHandsProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("idlehands.control.heartbeat")
    static let controlAgentEvent = Notification.Name("idlehands.control.agent")
}
