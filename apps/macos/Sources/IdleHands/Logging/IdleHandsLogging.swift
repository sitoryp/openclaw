...
enum IdleHandsLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = IdleHandsOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = IdleHandsFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        IdleHandsLogging.bootstrapIfNeeded()
        let label = IdleHandsLogging.makeLabel(subsystem: subsystem, category: category)
...
struct IdleHandsOSLogHandler: LogHandler {
...
struct IdleHandsFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = IdleHandsLogging.parseLabel(self.label)
