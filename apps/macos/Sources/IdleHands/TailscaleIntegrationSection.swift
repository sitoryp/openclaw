...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.idlehands/idlehands.json. Prefer IDLEHANDS_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.idlehands/idlehands.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.idlehands/idlehands.json. Restart the gateway to apply."
        }
