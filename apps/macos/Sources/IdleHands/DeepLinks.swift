...
private let deepLinkLogger = Logger(subsystem: "ai.idlehands", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "IdleHands is paused", message: "Unpause IdleHands to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run IdleHands agent?", message: body) else { return }
        }
