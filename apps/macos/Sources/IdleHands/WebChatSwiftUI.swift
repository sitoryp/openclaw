...
private let webChatSwiftLogger = Logger(subsystem: "ai.idlehands", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: IdleHandsChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> IdleHandsChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(IdleHandsChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> IdleHandsChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                IdleHandsGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    IdleHandsChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    IdleHandsAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<IdleHandsChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any IdleHandsChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = IdleHandsChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: IdleHandsChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<IdleHandsChatView>) -> NSViewController
    {
