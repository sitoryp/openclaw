...
enum CommandResolver {
    private static let projectRootDefaultsKey = "idlehands.gatewayProjectRootPath"
    private static let helperName = "idlehands"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let idlehandsEntry = root.appendingPathComponent("idlehands.mjs").path
        if FileManager().isReadableFile(atPath: idlehandsEntry) { return idlehandsEntry }
        let binEntry = root.appendingPathComponent("bin/idlehands.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/idlehands")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let idlehandsPaths = self.idlehandsManagedPaths(home: home)
        if !idlehandsPaths.isEmpty {
            extras.insert(contentsOf: idlehandsPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + idlehandsPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func idlehandsManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".idlehands"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func idlehandsExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectIdleHandsExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("idlehands.mjs").path,
            root.appendingPathComponent("bin/idlehands.js").path,
        ]
    ...
    static func hasAnyIdleHandsInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.idlehandsExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func idlehandsNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let idlehandsPath = self.projectIdleHandsExecutable(projectRoot: root) {
            return [idlehandsPath, subcommand] + extraArgs
        }
        if let idlehandsPath = self.idlehandsExecutable(searchPaths: searchPaths) {
            return [idlehandsPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "idlehands", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            idlehands entrypoint missing (looked for dist/index.js or idlehands.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real idlehands CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/idlehands"
        ...
        \(cliSection)
        \(projectSection)
        if command -v idlehands >/dev/null 2>&1; then
          CLI="$(command -v idlehands)"
          idlehands \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/idlehands.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/idlehands.mjs"
            node "$PRJ/idlehands.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/idlehands.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/idlehands.js"
            node "$PRJ/bin/idlehands.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent idlehands"
          pnpm --silent idlehands \(quotedArgs);
        else
          echo "idlehands CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? IdleHandsConfigFile.loadDict()
