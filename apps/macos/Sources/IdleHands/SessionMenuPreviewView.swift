...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.idlehands", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> IdleHandsSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: IdleHandsSessionPreviewEntry,
    ...
    private static func cache(payload: IdleHandsSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: IdleHandsChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [IdleHandsKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [IdleHandsKit.AnyCodable]) -> [IdleHandsChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(IdleHandsChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: IdleHandsChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: IdleHandsChatMessage) -> Bool {
    ...
    private static func toolNames(for message: IdleHandsChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: IdleHandsChatMessage) -> String? {
