...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["IDLEHANDS_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/idlehands")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("idlehands-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("idlehands-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/idlehands/ (rolling or stdout), or nil if none exist.
