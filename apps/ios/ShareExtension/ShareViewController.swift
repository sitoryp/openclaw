...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.idlehands.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to IdleHands", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to IdleHands gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to IdleHands.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "IdleHandsShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "IdleHands is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "IdleHands Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("idlehands-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
