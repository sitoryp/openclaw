...
final class RemindersService: RemindersServicing {
    func list(params: IdleHandsRemindersListParams) async throws -> IdleHandsRemindersListPayload {
        ...
        let payload: [IdleHandsReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return IdleHandsReminderPayload(
        ...
        return IdleHandsRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: IdleHandsRemindersAddParams) async throws -> IdleHandsRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = IdleHandsReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return IdleHandsRemindersAddPayload(reminder: payload)
    }
