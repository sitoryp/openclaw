...
@MainActor
final class IdleHandsAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.idlehands.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.idlehands.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.idlehands.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "idlehands.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "idlehands.watch.promptId"
    static let sessionKeyKey = "idlehands.watch.sessionKey"
    static let actionPrimaryIDKey = "idlehands.watch.action.primary.id"
    static let actionPrimaryLabelKey = "idlehands.watch.action.primary.label"
    static let actionSecondaryIDKey = "idlehands.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "idlehands.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "idlehands.watch.action.primary"
    static let actionSecondaryIdentifier = "idlehands.watch.action.secondary"
    static let actionIdentifierPrefix = "idlehands.watch.action."
    static let actionIDKeyPrefix = "idlehands.watch.action.id."
    static let actionLabelKeyPrefix = "idlehands.watch.action.label."
    static let categoryPrefix = "idlehands.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: IdleHandsWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> IdleHandsWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return IdleHandsWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "IdleHands" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [IdleHandsWatchAction]) -> [UNNotificationAction] {
...
@main
struct IdleHandsApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(IdleHandsAppDelegate.self) private var appDelegate
...
extension IdleHandsApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("IdleHands: installing uncaught exception handler")
