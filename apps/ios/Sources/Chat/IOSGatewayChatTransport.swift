...
struct IOSGatewayChatTransport: IdleHandsChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.idlehands", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> IdleHandsChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(IdleHandsChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> IdleHandsChatHistoryPayload {
        ...
        return try JSONDecoder().decode(IdleHandsChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [IdleHandsChatAttachmentPayload]) async throws -> IdleHandsChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [IdleHandsChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(IdleHandsChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(IdleHandsGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: IdleHandsGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: IdleHandsChatEventPayload.self)
                        {
