...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__IDLEHANDS_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<IdleHandsTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.idlehands", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = IdleHandsTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> IdleHandsTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return IdleHandsTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> IdleHandsTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> IdleHandsTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return IdleHandsTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> IdleHandsTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = IdleHandsTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: IdleHandsTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: IdleHandsAgentEventPayload.self
            ) else {
                continue
            }
