...
final class MotionService: MotionServicing {
    func activities(params: IdleHandsMotionActivityParams) async throws -> IdleHandsMotionActivityPayload {
        ...
        let mapped: [IdleHandsMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return IdleHandsMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: IdleHandsPedometerParams) async throws -> IdleHandsPedometerPayload {
        ...
        let payload: IdleHandsPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = IdleHandsPedometerPayload(
