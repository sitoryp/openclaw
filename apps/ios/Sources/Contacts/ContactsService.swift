...
final class ContactsService: ContactsServicing {
    ...
    func search(params: IdleHandsContactsSearchParams) async throws -> IdleHandsContactsSearchPayload {
        ...
        return IdleHandsContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: IdleHandsContactsAddParams) async throws -> IdleHandsContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return IdleHandsContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return IdleHandsContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> IdleHandsContactPayload {
        IdleHandsContactPayload(
