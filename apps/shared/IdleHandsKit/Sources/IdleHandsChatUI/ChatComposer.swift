...
@MainActor
struct IdleHandsChatComposer: View {
    @Bindable var viewModel: IdleHandsChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(IdleHandsChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(IdleHandsChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \IdleHandsPendingAttachment.id)
                { (att: IdleHandsPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            IdleHandsPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(IdleHandsChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(IdleHandsChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(IdleHandsChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(IdleHandsChatTheme.subtleCard)
        .clipShape(Capsule())
    }
