...
private let chatUILogger = Logger(subsystem: "ai.idlehands", category: "IdleHandsChatUI")

@MainActor
@Observable
public final class IdleHandsChatViewModel {
    public private(set) var messages: [IdleHandsChatMessage] = []
    ...
    public var attachments: [IdleHandsPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [IdleHandsChatPendingToolCall] = []
    public private(set) var sessions: [IdleHandsChatSessionEntry] = []
    private let transport: any IdleHandsChatTransport
    ...
    private var pendingToolCallsById: [String: IdleHandsChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any IdleHandsChatTransport) {
    ...
    public var sessionChoices: [IdleHandsChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [IdleHandsChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: IdleHandsPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [IdleHandsChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: IdleHandsChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: IdleHandsChatMessage) -> IdleHandsChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> IdleHandsChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return IdleHandsChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: IdleHandsChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [IdleHandsChatMessage],
        incoming: [IdleHandsChatMessage]) -> [IdleHandsChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [IdleHandsChatMessage]) -> [IdleHandsChatMessage] {
        var result: [IdleHandsChatMessage] = []
    ...
    private static func dedupeKey(for message: IdleHandsChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [IdleHandsChatMessageContent] = [
            IdleHandsChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> IdleHandsChatAttachmentPayload in
            IdleHandsChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> IdleHandsChatSessionEntry {
        IdleHandsChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: IdleHandsChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: IdleHandsChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: IdleHandsAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = IdleHandsChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> IdleHandsPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
