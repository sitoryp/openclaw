...
enum IdleHandsChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("IdleHandsChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("IdleHandsChatTheme.onboardingAssistantBubble"),
...
enum IdleHandsPlatformImageFactory {
    static func image(_ image: IdleHandsPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
