...
struct ChatMessageBubble: View {
    let message: IdleHandsChatMessage
    let style: IdleHandsChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? IdleHandsChatTheme.userText : IdleHandsChatTheme.assistantText
    ...
    private var inlineAttachments: [IdleHandsChatMessageContent] {
    ...
    private var toolCalls: [IdleHandsChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [IdleHandsChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? IdleHandsChatTheme.userBubble
        }
        if self.style == .onboarding {
            return IdleHandsChatTheme.onboardingAssistantBubble
        }
        return IdleHandsChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return IdleHandsChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: IdleHandsChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? IdleHandsChatTheme.userText : IdleHandsChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: IdleHandsChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(IdleHandsChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(IdleHandsChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [IdleHandsChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: IdleHandsChatTheme.assistantText)
            }
        }
    }
}
