...
import Foundation
import Testing
@testable import IdleHandsChatUI
...
private final class TestChatTransport: @unchecked Sendable, IdleHandsChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [IdleHandsChatHistoryPayload]
    private let sessionsResponses: [IdleHandsChatSessionsListResponse]

    private let stream: AsyncStream<IdleHandsChatTransportEvent>
    private let continuation: AsyncStream<IdleHandsChatTransportEvent>.Continuation

    init(
        historyResponses: [IdleHandsChatHistoryPayload],
        sessionsResponses: [IdleHandsChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<IdleHandsChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> IdleHandsChatHistoryPayload {
        ...
        return self.historyResponses.last ?? IdleHandsChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [IdleHandsChatAttachmentPayload]) async throws -> IdleHandsChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return IdleHandsChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> IdleHandsChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? IdleHandsChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: IdleHandsChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = IdleHandsChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = IdleHandsChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { IdleHandsChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = IdleHandsChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = IdleHandsChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = IdleHandsChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { IdleHandsChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
